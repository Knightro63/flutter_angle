...
static void flutter_angle_plugin_handle_method_call(FlutterAnglePlugin *self, FlMethodCall *method_call){
  g_autoptr(FlMethodResponse) response = nullptr;
    ...
    auto cuttentTexture = OpenGLRenderer(
    ...
    auto textureId = cuttentTexture->textureId;
    g_autoptr(FlValue) value = cuttentTexture->createTexture();
    response = FL_METHOD_RESPONSE(fl_method_success_response_new(value));

    auto [it1, inserted1] = self->renderers.try_emplace(
      textureId, 
      std::make_unique<OpenGLRenderer>(std::move(currentTexture))
    ); 
    
    if (inserted1) {
      std::cout << "Successfully inserted renderer for key " << textureId << std::endl;
    } 
    else {
      std::cout << "Renderer for key " << textureId << " already exists." << std::endl;
    }
  }
    ...
    auto currentTexture = self->renderers[textureId].get();
    ...
    self->renderers[textureId].release();
